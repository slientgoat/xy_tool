defmodule Tool do
  ...
  def seconds_to_hms(seconds) do
    ...
    "#{String.pad_leading(to_string(h), 2, "0")}:#{String.pad_leading(to_string(m), 2, "0")}:#{
      String.pad_leading(to_string(res_sec2), 2, "0")
    }"
  end
  ...
  @doc """
    ip转int
  """
  def ip_to_int({a, b, c, d}), do: a * 16_777_216 + b * 65536 + c * 256 + d

  def ip_to_int(var),
    do: String.split(var, ".") |> Enum.map(&String.to_integer/1) |> List.to_tuple() |> ip_to_int()
  ...
  def int_to_ip(var, :tuple),
    do: {bsr(var, 24), bsr(band(var, 16_711_680), 16), bsr(band(var, 65280), 8), band(var, 255)}
  ...
  def gen_order_id(n \\ 6) do
    {{y, mon, d}, {h, m, s}} =
      NaiveDateTime.to_erl(Timex.shift(NaiveDateTime.utc_now(), hours: 8))

    stime =
      fitzero(y, 4) <>
        fitzero(mon, 2) <> fitzero(d, 2) <> fitzero(h, 2) <> fitzero(m, 2) <> fitzero(s, 2)
